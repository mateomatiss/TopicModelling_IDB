...
@author: Mateo
'''
import tweepy  # @UnresolvedImport
from textblob import TextBlob  # @UnresolvedImport

consumer_key = 'NIq1OaHGPXCnLHytMnyErVaol'
consumer_secret = '3ksmjLDuOkIypy9gcPdnQ7eHzUxYPvwrcAyxsgs6PgLpxpzeqI'

access_token = '840485029113389056-fcDqc2EA6t3yDuPF9ByWqncnmeL3aiu'
access_token_secret = 'vXFJ5x0TfoAoJa5KgYU0JtLRcrxMZrM1Kb7ZpuIC3EROP'
...
public_tweets = api.search('Erdogan', count=100)
...
def read_data(path, reviews, labels, words):
    #if True:#for tweet in public_tweets:
    for tweet in public_tweets:#
        analysis = TextBlob(tweet.text)
        if analysis.sentiment.polarity != 0.0:
        #    tweet = public_tweets[0]
            clean_content = clean_text(tweet.text)
            [words.append(w) for w in clean_content.split()]     
            reviews.append(clean_content)
            labels.append(analysis.sentiment)


def read_data2(path, reviews, labels, words):
...
tfidf_vectorizer = TfidfVectorizer(max_df=0.5, min_df=0.05, #max_df=0.95, min_df=0.01,
...
tf_vectorizer = CountVectorizer(max_df=0.5, min_df=0.05, #max_df=0.95, min_df=0.01,
                                max_features=n_features,
                                tokenizer=tokenize, stop_words=STOPSET)
t0 = time()
